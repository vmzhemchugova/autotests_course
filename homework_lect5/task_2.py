...
def repeats(our_str):
    """
    Функция проверяет сколько раз каждый символ уже встречался во входной строке,
    и добавляет количество к символам с помощью постфикса формата _n
    :param our_str: строка.
    :return: строка из букв их частот в строке от начала строки до текущей буквы, разделенных _
    """
    new_str = []
    for i in range(len(our_str)):
        new_str.append("{}_{}".format(our_str[i], our_str[0: i + 1].count(our_str[i])))
