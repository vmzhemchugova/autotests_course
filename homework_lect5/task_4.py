...
def scrabble(word):
    """
    Функция помогает рассчитать кол-во очков (points),
    полученное игроком за слово (word)
    :param word: слово в формате строки.
    :return: количество баллов за слово (число).
    """
    word = word.replace('ё', 'е')
    dict_points = {1: ['а', 'в', 'е', 'и', 'н', 'о', 'р', 'с', 'т'], 2: ['д', 'к', 'л', 'м', 'п', 'у'],
                   3: ['б', 'г', 'ь', 'я'], 4: ['й', 'ы'], 5: ['ж', 'з', 'х', 'ц', 'ч'], 8: ['ф', 'ш', 'э', 'ю'],
                   10: ['щ'],
                   15: ['ъ']}
    points = 0
    for i in range(len(word)):
        for point, char in dict_points.items():
            if word[i] in char:
                points += point
    return points
