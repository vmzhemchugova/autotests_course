...
def treatment_sum(our_tuple: tuple):
    """
    Функция принимает на вход кортеж и производит проверку:
    Если в кортеже 2 элемента и их можно сложить - возвращает получившийся результат;
    Если в кортеже 2 элемента и их нельзя сложить - обрабатывает исключение и возвращает строку 'Нельзя сложить эти данные';
    Если в кортеже меньше двух элементов - обрабатывает исключение и возвращает строку 'Недостаточно данных';
    Если в кортеже больше двух элементов - генерирует исключение Exception с текстом 'Много данных'.
    :param our_tuple: кортеж входных данных,
    :return: результат сложения или обработанное исключение
    """
    try:
        if len(our_tuple) > 2:
