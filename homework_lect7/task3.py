...
class PublicTransport:
    ...
    def info(self):
        """
        Вывод на печать информации о марке, цвете, годе выпуска и мощности двигателя
        """
        print(f'Автомобиль марки: {self.brand} представлен в цвете {self.color} {self.year} года выпуска. '
              f'Мощность двигателя данной модели составляет {self._engine_power}')
...
class Bus(PublicTransport):
    ...
    @park.setter
    def park(self, park_bus):
        """
        Присвоение номера парку и проверка, что он в диапазоне от 1000 до 9999
        :param park_bus: новый номер парка
        """
        assert 1000 <= park_bus <= 9999
        self.__park = park_bus
...
class Tram(PublicTransport):
    ...
    def how_long(self):
        """
        Вычисление времени на прохождение маршрута по формуле max_speed/(4*path)
